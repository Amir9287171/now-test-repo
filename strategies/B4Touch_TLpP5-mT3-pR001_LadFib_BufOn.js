/**
 * @filename B4Touch_TLpP5-mT3-pR001_LadFib_BufOn.js
 * @description خرید در برخورد و بازگشت قیمت از یک خط روند صعودی (حمایت) — بر خلاف استراتژی‌های شکست، اینجا ورود دقیقاً روی لمس و بازگشت خط انجام می‌شود، نه شکست آن. pivotPeriod=5، minTouchPoints=3، precision=0.001 | حد سود ممنوع (حذف شد)، خروج فقط با حد ضرر پلکانی = فیبوناچی تعدیل‌شده (Adjusted-Fibonacci)، بافر فعال (enableSmartContinuation: true)
 */
...
function customStrategy(data, index, breakPointsParam, _ichimokuUnused, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_B4Touch_TLpP5_mT3_pR001_LadFib_BufOn || globalThis.__state_B4Touch_TLpP5_mT3_pR001_LadFib_BufOn.dataRef !== data) {
    globalThis.__state_B4Touch_TLpP5_mT3_pR001_LadFib_BufOn = { dataRef: data, lastTouchIndex: {} };
  }
  const st = globalThis.__state_B4Touch_TLpP5_mT3_pR001_LadFib_BufOn;

  const activeLines = trendLinesParam || getTrendLines();
  if (activeLines.length === 0) return null;

  // خطوط صعودی (حمایت) — به دنبال «برخورد و بازگشت»، نه شکست
  ...
  const TOUCH_TOLERANCE = 0.0015; // 0.15% فاصله‌ی مجاز کندل از خط برای «لمس» محسوب شدن
  ...
  for (const line of upLines) {
    if (isTrendLineBroken(line, index)) continue; // خط شکسته دیگر حمایت معتبر نیست
    if (index < line.startIndex) continue;
    ...
    // شرط بازگشت: کندل صعودی و بسته‌شدن بالای خط (یعنی خط را نشکسته)
    if (candle.close <= candle.open) continue;
    if (candle.close <= lineValue) continue;

    // جلوگیری از سیگنال تکراری روی همان خط با فاصله‌ی کم
    const lastIdx = st.lastTouchIndex[line.id];
    if (lastIdx !== undefined && index - lastIdx < 5) continue;
  ...
  st.lastTouchIndex[bestLine.id] = index;
