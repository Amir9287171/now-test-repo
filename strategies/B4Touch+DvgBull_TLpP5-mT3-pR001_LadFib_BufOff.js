/**
 * @filename B4Touch+DvgBull_TLpP5-mT3-pR001_LadFib_BufOff.js
 * @description خرید فقط زمانی که هم‌زمان (۱) قیمت یک خط روند صعودی را لمس و از آن بازگشت کند و (۲) واگرایی صعودی روی همان کندل تایید شود — ترکیب دو سیگنال برای فیلتر قوی‌تر | حد سود ممنوع (حذف شد)، خروج فقط با حد ضرر پلکانی = فیبوناچی تعدیل‌شده (Adjusted-Fibonacci)، بافر غیرفعال (enableSmartContinuation: false)
 */
...
function customStrategy(data, index, breakPointsParam, _ichimokuUnused, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadFib_BufOff || globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadFib_BufOff.dataRef !== data) {
    globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadFib_BufOff = { dataRef: data, lastTouchIndex: {} };
  }
  const st = globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadFib_BufOff;
  ...
  const hasBullishDivergence = divSignals.some(sig =>
    (sig.type === 'RegularBullish' || sig.type === 'HiddenBullish') && sig.endIndex === index
  );
  ...
  for (const line of upLines) {
    if (isTrendLineBroken(line, index)) continue;
