/**
 * @filename S4Ic14-30-57_TLpP5-mT3-pR001_LadFib_BufOff.js
 * @description فروش با تایید ایچیموکو (14,30,57)، حد ضرر 0.4%، خط روند با pivotPeriod=5، minTouchPoints=3، precision=0.001 | اصلاح‌شده: حد سود حذف شد، حد ضرر پلکانی = فیبوناچی تعدیل‌شده (Adjusted-Fibonacci)، بافر غیرفعال (enableSmartContinuation: false)
 */
...
function customStrategy(data, index, breakPointsParam, ichimokuParam, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_S4Ic14_30_57_TLpP5_mT3_pR001_LadFib_BufOff || globalThis.__state_S4Ic14_30_57_TLpP5_mT3_pR001_LadFib_BufOff.dataRef !== data) {
    globalThis.__state_S4Ic14_30_57_TLpP5_mT3_pR001_LadFib_BufOff = { dataRef: data, brokenLines: new Set() };
  }
  const brokenLines = globalThis.__state_S4Ic14_30_57_TLpP5_mT3_pR001_LadFib_BufOff.brokenLines;
  ...
  // برای فروش، نزدیک‌ترین شکست به 0.12% (همان هدف)
  const TARGET = 0.12;
  ...
  for (const breakInfo of downBreaks) {
    ...
    const diffPercent = ((lineValue - breakPrice) / lineValue) * 100; // برای فروش، درصد شکست رو به پایین
