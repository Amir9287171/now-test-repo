/**
 * @filename B4Touch+DvgBull_TLpP5-mT3-pR001_LadLine_BufOn.js
 * @description خرید فقط زمانی که هم‌زمان (۱) قیمت یک خط روند صعودی را لمس و از آن بازگشت کند و (۲) واگرایی صعودی روی همان کندل تایید شود — ترکیب دو سیگنال برای فیلتر قوی‌تر | حد سود ممنوع (حذف شد)، خروج فقط با حد ضرر پلکانی = خطی پویا (Dynamic-Linear)، بافر فعال (enableSmartContinuation: true)
 */
...
function customStrategy(data, index, breakPointsParam, ichimokuParam, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadLine_BufOn || globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadLine_BufOn.dataRef !== data) {
    globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadLine_BufOn = { dataRef: data, lastTouchIndex: {} };
  }
  const st = globalThis.__state_B4Touch_DvgBull_TLpP5_mT3_pR001_LadLine_BufOn;
