/**
 * @filename B4DvgBull_Default.js
 * @description BUY با تایید واگرایی صعودی خالص (بدون خط روند/ایچیموکو).
 *   اندیکاتور(ها): RSI و MACD | نوع سیگنال پذیرفته‌شده: Regular و Hidden هردو
 *   خط پایه (Control) — بدون هیچ override‌ای؛ دقیقاً پیش‌فرض‌های سیستم (RSI+MACD، PIVOT_PERIOD=3 و بقیه‌ی مقادیر داخلی divergence-detector.js). بدون این، معنای «بهتر شدن» بقیه‌ی ترکیب‌ها قابل سنجش نیست.
...
const ANALYSIS_CONFIG = {
  ...
  divergence: {
    indicators: ["RSI", "MACD"],
    // config تعریف نشده → تمام پارامترهای داخلی DIVERGENCE_CONFIG (پیش‌فرض سیستم) استفاده می‌شود
  }
};
