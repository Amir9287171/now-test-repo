/**
 * @filename B4Ic14-30-57_TLpP5-mT3-pR04_BrT012_INVERSE_LadLine_BufOn.js
 * @description معکوس‌سازی استراتژی خرید: فروش در همان نقطه‌ای که خرید می‌کرد | اصلاح‌شده: حد سود حذف شد، حد ضرر پلکانی = خطی پویا (Dynamic-Linear)، بافر فعال (enableSmartContinuation: true)
 */
...
function customStrategy(data, index, breakPointsParam, ichimokuParam, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_S4_INVERSE_LadLine_BufOn || globalThis.__state_S4_INVERSE_LadLine_BufOn.dataRef !== data) {
    globalThis.__state_S4_INVERSE_LadLine_BufOn = { dataRef: data, brokenLines: new Set() };
  }
  const brokenLines = globalThis.__state_S4_INVERSE_LadLine_BufOn.brokenLines;
