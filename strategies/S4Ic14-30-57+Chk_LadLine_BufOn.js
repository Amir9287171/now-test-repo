/**
 * @filename S4Ic14-30-57+Chk_LadLine_BufOn.js
 * @description فروش با تاییدیه‌ی سه‌گانه‌ی کامل ایچیموکو (14,30,57): قیمت زیر ابر + تنکان زیر کیجون + چیکو نزولی — سیگنال فقط در همان کندلی که هر سه شرط تازه هم‌راستا شده‌اند صادر می‌شود | حد سود ممنوع (حذف شد)، خروج فقط با حد ضرر پلکانی = خطی پویا (Dynamic-Linear)، بافر فعال (enableSmartContinuation: true)
 */
...
function customStrategy(data, index, breakPointsParam, ichimokuParam, trendLinesParam, refineEntryPrice) {
  ...
  if (!globalThis.__state_S4Ic14_30_57_Chk_LadLine_BufOn || globalThis.__state_S4Ic14_30_57_Chk_LadLine_BufOn.dataRef !== data) {
    globalThis.__state_S4Ic14_30_57_Chk_LadLine_BufOn = { dataRef: data, wasBearish: false };
  }
  const st = globalThis.__state_S4Ic14_30_57_Chk_LadLine_BufOn;
  ...
  const isBearishNow = ichimokuParam.isPriceBelowCloud &&
                        !ichimokuParam.isTenkanAboveKijun &&
                        ichimokuParam.isChikouBullish === false;
